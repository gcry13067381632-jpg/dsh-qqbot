...
**🧠 悬浮球「用户代发」，还能写进 bot 记忆**
设置面板的悬浮球 dock 能直接以机器人身份往群/私聊发消息（勾一下 🧠）。勾选后，发完会把这条消息作为**模拟用户消息**记进目标会话的上下文——内容带「用户代你发送」标记，只进 Web 会话流，**不唤醒、不开新回合**；bot 下次真正被 @ 时才自然读到。目标会话被闲置回收了会自动恢复再写；LLM 正在思考/流式输出时会自动跳过，绝不打断生成中的回合（此前因格式写错把会话写坏的坑已按官方事件契约修正）。

**❓ 提问变成 QQ 按钮卡片**
机器人问 `ask_user_question` 时，选项直接以按钮卡片发到当前对话者的 QQ：群聊里只有被问的那位能点（别人点会被拒），Web 浮层与 QQ 卡片任一端作答即结算。

**✅ 远程审批，Web 也能结算**
权限审批除了在 QQ 里回一次性 CODE，Web 面板也能直接放行/拒绝：跨实例待审汇总、点击即结算，QQ 与 Web 两端状态同步。

**🔧 健壮性加固**
面板代发写入前先过「回合活跃」安全闸、会话被回收时自动 getOrCreate 恢复；模拟消息严格按 dsh 官方 user/message 契约生成，和真人消息同构。
